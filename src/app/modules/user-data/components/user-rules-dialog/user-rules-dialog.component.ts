...
export class UserRulesDialogComponent {
  ...
  constructor(public dialogRef: MatDialogRef<UserRulesDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: UserRulesDialogData, private rulesService: RulesService, private snackBarService: SnackBarService) {
    ...
    this.pointsInTieBreak = new FormControl({ value: this.data.rules.pointsInTieBreak, disabled: this.editingDisabled }, [Validators.required]);
    this.matchTermination = new FormControl({ value: this.data.rules.matchTermination, disabled: this.editingDisabled }, [Validators.required]);
    this.teamTimeoutsPerSet = new FormControl({ value: this.data.rules.teamTimeoutsPerSet, disabled: this.editingDisabled }, [Validators.required]);
    this.teamTimeoutDuration = new FormControl({ value: this.data.rules.teamTimeoutDuration, disabled: this.editingDisabled }, [Validators.required]);
    this.technicalTimeoutDuration = new FormControl({ value: this.data.rules.technicalTimeoutDuration, disabled: this.editingDisabled }, [Validators.required]);
    this.gameIntervalDuration = new FormControl({ value: this.data.rules.gameIntervalDuration, disabled: this.editingDisabled }, [Validators.required]);
    this.beachCourtSwitchFreq = new FormControl({ value: this.data.rules.beachCourtSwitchFreq, disabled: this.editingDisabled }, [Validators.required]);
    this.beachCourtSwitchFreqTieBreak = new FormControl({ value: this.data.rules.beachCourtSwitchFreqTieBreak, disabled: this.editingDisabled }, [Validators.required]);
    this.substitutionsLimitation = new FormControl({ value: this.data.rules.substitutionsLimitation, disabled: this.editingDisabled }, [Validators.required]);
    this.teamSubstitutionsPerSet = new FormControl({ value: this.data.rules.teamSubstitutionsPerSet, disabled: this.editingDisabled }, [Validators.required]);
    this.customConsecutiveServesPerPlayer = new FormControl({ value: this.data.rules.customConsecutiveServesPerPlayer, disabled: this.editingDisabled }, [Validators.required]);
  ...
  get nameFormControl() { return this.rulesFormGroup.get('name'); }
  get tieBreakInLastSetFormControl() { return this.rulesFormGroup.get('tieBreakInLastSet'); }
  get twoPointsDifferenceFormControl() { return this.rulesFormGroup.get('twoPointsDifference'); }
  get sanctionsFormControl() { return this.rulesFormGroup.get('sanctions'); }
  get teamTimeoutsFormControl() { return this.rulesFormGroup.get('teamTimeouts'); }
  get technicalTimeoutsFormControl() { return this.rulesFormGroup.get('technicalTimeouts'); }
  get gameIntervalsFormControl() { return this.rulesFormGroup.get('gameIntervals'); }
  get beachCourtSwitchesFormControl() { return this.rulesFormGroup.get('beachCourtSwitches'); }

  private createOptions(): void {
    this.setsPerGameOptions = [ {value: 5, label: 'Game of 5 sets'}, {value: 3, label: 'Game of 3 sets'}, {value: 1, label: 'Game of 1 set'} ];
  ...
  onSubmitForm(): void {
    ...
    if (this.data.crudType === CrudType.Create) {
      this.rulesService.createRules(rules).subscribe(_rules => this.onValidResponse(), _error => this.onInvalidResponse());
    } else if (this.data.crudType === CrudType.Update) {
      this.rulesService.updateRules(rules).subscribe(_rules => this.onValidResponse(), _error => this.onInvalidResponse());
    }
  }
