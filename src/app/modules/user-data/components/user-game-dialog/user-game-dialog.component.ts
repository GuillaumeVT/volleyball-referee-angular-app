...
export class UserGameDialogComponent {
  ...
  constructor(public dialogRef: MatDialogRef<UserGameDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: UserGameDialogData,
    ...
    }, [this.teamsMustBeDifferentValidator, this.leagueMustExistValidator.bind(this), this.homeTeamMustExistValidator.bind(this), this.guestTeamMustExistValidator.bind(this), this.rulesMustExistValidator.bind(this)]);
  ...
  private teamsMustBeDifferentValidator(abstractControl: AbstractControl): { sameTeams: boolean } {
    if ((abstractControl.get('homeTeam').value && abstractControl.get('guestTeam').value)
  ...
  onSubmitForm(): void  {
    ...
    game.gender = this.homeTeamFormControl.value.gender === this.guestTeamFormControl.value.gender ? this.homeTeamFormControl.value.gender : 'MIXED';
    ...
    if (this.data.crudType === CrudType.Create) {
      this.gameService.createGame(game).subscribe(_game => this.onValidResponse(), _error => this.onInvalidResponse());
    } else if (this.data.crudType === CrudType.Update) {
      this.gameService.updateGame(game).subscribe(_game => this.onValidResponse(), _error => this.onInvalidResponse());
    }
  }
